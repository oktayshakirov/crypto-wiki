...
const PersonQuickFacts = ({ facts, title, compact = false }) => {
  ...
  const lastRowStart =
    rows.length % 2 === 0 ? rows.length - 2 : rows.length - 1;
    ...
    <div className="mb-10 rounded-lg border border-gray-300 p-5 text-start dark:border-gray-700">
      ...
      <dl className="grid grid-cols-1 gap-x-8 gap-y-3 sm:grid-cols-2">
        {rows.map(([key, label], i) => (
          <div
            key={key}
            className={`flex flex-col border-b border-gray-200 pb-2 last:border-b-0 last:pb-0 sm:flex-row sm:items-baseline dark:border-gray-800 ${
