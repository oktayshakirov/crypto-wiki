...
export const getSinglePage = (folder) => {
  ...
  // Apply different sorting strategies based on the folder name
  let sortedPages;
  if (folder.includes("crypto-ogs") || folder.includes("exchanges")) {
    // Sort by order for "crypto-ogs" and "exchanges"
    sortedPages = sortByOrder(publishedPages);
  } else {
    // Sort by date for other posts
