...
class BitmediaManager {
  ...
  initializeAdsForRoute(route) {
    ...
    // Determine delay based on whether this is a route change
    const isRouteChange = previousRoute && previousRoute !== route;
    const delay = isRouteChange ? 1000 : 500; // Longer delay after route change
    ...
    this.initializationTimeout = setTimeout(() => {
      try {
        ...
        this.adInstances.forEach((instance, adId) => {
          try {
            if (
              ...
              let insElement = instance.container.querySelector(`ins#${adId}`);
              if (!insElement) {
        ...
        // Use longer delays to ensure Bitmedia has time to process each ad
        adsToInitialize.forEach(({ adId, instance, insElement }, index) => {
          ...
          }, index * 400); // Longer stagger to give Bitmedia time to process
        });
