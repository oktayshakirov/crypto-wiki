...
export const organizationSchema = () => ({
  ...
  sameAs: [
    "https://x.com/oktayshakirov",
    "https://www.linkedin.com/in/oktayshakirov/",
    "https://www.youtube.com/@TheCrypto_Wiki",
  ],
});
...
export const exchangeReviewSchema = ({
  ...
  // `founded` is a readable label ("2015 (public launch in Germany, January
  // 2019)"), but schema.org foundingDate expects a Date - so only a clean
  // leading 4-digit year is passed through, same as personSchema does for `born`.
  const foundedYear = /^\s*(\d{4})\b/.exec(quickFacts.founded || "");
  ...
  const itemReviewed = {
    ...
    foundingDate: foundedYear ? foundedYear[1] : undefined,
...
export const personSchema = ({
  ...
  // `born` is free text ("1965, Lincoln, Nebraska, USA") so it is not safe to
  // emit as a bare birthDate; only pass through a clean leading 4-digit year.
  const bornYear = /^\s*(\d{4})\b/.exec(quickFacts.born || "");
  ...
  const schema = {
    ...
    birthDate: bornYear ? bornYear[1] : undefined,
    birthPlace: quickFacts.birthplace || undefined,
    nationality: quickFacts.nationality || undefined,
    alumniOf: quickFacts.education || undefined,
    jobTitle: quickFacts.role || undefined,
    knowsAbout: quickFacts.knownFor || undefined,
  };
...
// An embedded YouTube video. Emitted only on pages that actually render the
// player. Note what this does *not* buy: since Google's August 2023 change the
// video rich result is shown mainly where video is the page's main content, and
// an article with a supplementary embed generally does not qualify. This feeds
// the Video tab and Discover - do not budget for a search thumbnail.
//
...
export const videoObjectSchema = (video, { withClips = false } = {}) => {
  if (!video) return null;
  const contentUrl = `https://www.youtube.com/watch?v=${video.id}`;
  const schema = {
    "@context": "https://schema.org",
    "@type": "VideoObject",
    name: plainify(video.title),
    description: plainify(video.description),
  ...
  // will not surface them. The chapter data is already validated on render by
  // `meta.check_chapters` in the video repo.
  const chapters = video.chapters || [];
  if (withClips && chapters.length) {
    schema.hasPart = chapters.map((chapter, i) => ({
      "@type": "Clip",
      name: plainify(chapter.title),
      startOffset: chapter.start,
      endOffset:
        i + 1 < chapters.length ? chapters[i + 1].start : video.seconds,
