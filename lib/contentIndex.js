...
//     apps poll to discover new content
//   - scripts/syncContent.js          -> writes those entries into Firestore
//   - the URL a push notification links to (mirrored in each app's
...
const SECTIONS = [
  { dir: "content/posts", collection: "posts", urlPath: "/posts" },
  { dir: "content/exchanges", collection: "exchanges", urlPath: "/exchanges" },
  {
    dir: "content/crypto-ogs",
    collection: "crypto-ogs",
    urlPath: "/crypto-ogs",
  },
];
...
function readSection(root, section) {
  ...
  return fs
    ...
    .map((file) => {
      ...
      // contentParser lets frontmatter override the URL, so mirror that here.
      const slug = data.url
        ? data.url.replace(/^\//, "")
        : file.replace(/\.mdx?$/, "");
 ...
 *
 * They live in json/videos.json because the id, uploadDate and duration only
 * exist once a video is uploaded. Only long form is listed: those are the ones
 * with a page at /videos/<slug>, which is where a notification has to land.
 * `date` is the upload date, so the app's feed orders them against posts
 * correctly.
 */
function readVideos(root) {
  const file = path.join(root, "json/videos.json");
  ...
  return videos
    ...
    .map((video) => ({
      ...
      date: parsePublishedAt(video.uploadDate)?.toISOString() ?? null,
    }));
}
